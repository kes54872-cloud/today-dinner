...
import { Button, EmptyState, FoodImage, Icon, ProgressBar } from '../components/ui'
        ...
        {sel && (
          <>
            <p className="mt-1 text-sm text-ink">
              ...
              <span className="num text-primary">{sel.usedCount}개</span>를 사용해요
            </p>
            <div className="mt-4 flex items-end gap-3">
              <span className="num text-4xl font-extrabold text-primary">{usePct}%</span>
              <span className="pb-1 text-sm text-muted">활용 가능</span>
            </div>
            <ProgressBar value={usePct} label="냉장고 활용도" className="mt-2" />
          </>
        )}
      </div>

      {/* 메뉴 목록 */}
      <h2 className="mb-3 mt-8 text-lg font-bold text-ink">가장 많이 활용할 수 있는 메뉴</h2>
        ...
        {ranked.map((m) => (
          <li key={m.id}>
            ...
            >
              ...
              <Link to={`/menu/${m.id}`} aria-label={`${m.name} 레시피`} onClick={(e) => e.stopPropagation()} className="shrink-0 text-muted hover:text-ink">
...
function BackLink() {
  return (
    <Link to="/fridge" className="mb-3 inline-flex items-center gap-1 text-sm text-muted hover:text-ink">
