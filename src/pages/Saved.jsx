...
export default function Saved() {
  ...
  return (
    <PageContainer>
      <h1 className="text-2xl font-extrabold text-ink md:text-3xl">찜한 메뉴</h1>
        ...
        <>
          ...
          <div className="mt-5 space-y-8">
            {showCook && cookItems.length > 0 && (
              <section>
                <h2 className="mb-3 text-sm font-bold text-muted">해먹기 {cookItems.length}</h2>
            ...
            {showDelivery && deliveryItems.length > 0 && (
              <section>
                <h2 className="mb-3 text-sm font-bold text-muted">배달 {deliveryItems.length}</h2>
            ...
            {tab === '해먹기' && cookItems.length === 0 && (
              <EmptyState icon="🍳" title="저장한 해먹기 메뉴가 없어요." />
            )}
            {tab === '배달' && deliveryItems.length === 0 && (
              <EmptyState icon="🛵" title="저장한 배달 메뉴가 없어요." />
            )}
          </div>
        </>
      )}
