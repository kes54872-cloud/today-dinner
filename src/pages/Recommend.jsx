...
const FILTERS = [
  ...
  { key: 'fridge', label: '냉장고 재료 우선', test: (m) => (m.fridgeUse?.length ?? 0) >= 3 },
  { key: 'new', label: '새로운 요리', test: (m) => m.category === 'new' },
]

const SECTIONS = [
  { key: 'safe', title: '안전한 선택', caption: '실패 없이 내가 좋아할 메뉴' },
  { key: 'new', title: '새로운 선택', caption: '평소 취향에서 조금 벗어난 메뉴' },
  { key: 'challenge', title: '한 단계 도전', caption: '지금 할 수 있는 요리보다 조금 어려운 메뉴' },
]
    ...
    <PageContainer>
      <div className="animate-rise">
        ...
        <p className="mt-1.5 text-sm text-muted">
          취향·냉장고·요리 실력을 반영한 <span className="num">{total}</span>개의 메뉴예요.
        </p>
      </div>
      ...
      <div className="no-scrollbar -mx-4 mt-5 flex gap-2 overflow-x-auto px-4 pb-1">
        {FILTERS.map((f) => (
          <Chip key={f.key} active={active.includes(f.key)} onClick={() => toggle(f.key)}>
            {f.label}
          </Chip>
        ))}
        {active.length > 0 && (
